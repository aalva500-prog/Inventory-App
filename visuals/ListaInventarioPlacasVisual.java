...
import services.ServicioInventarioPlaca;
...
public class ListaInventarioPlacasVisual extends JFrame {
	...
	public ListaInventarioPlacasVisual() {
		super();
		initialize();
	...
	private void initialize() {
			...
			this.setTitle("Listado de Inventarios de Placas de Filtro");
			...
			LinkedList<InventarioPlacas> list = new LinkedList<InventarioPlacas>();
			try {
				list = ServicioInventarioPlaca.getInventarioPlacas();
			...
			for (int i = 0; i < list.size(); i++) {
				columnDataProducto.add(list.get(i).getFechainventarioplacas());
				}
			defaultTableModel.setRowCount(list.size());							
			defaultTableModel.addColumn("Fechas de los Inventarios de Placas de Filtro",columnDataProducto.toArray());
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					...
					try {
						LinkedList<InventarioPlacas> users = ServicioInventarioPlaca.getInventarioPlacas();
						InventarioPlacas u = users.get(pos);
						DatejSpinner.setValue(u.getFechainventarioplacas());						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					} catch (ClassNotFoundException e1) {
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			jLabel = new JLabel();
			jLabel.setText("Fecha del Inventario de Placas de Filtro:");
			jLabel.setBounds(new Rectangle(5, 4, 235, 26));
			jPanel = new JPanel();
		...
		if (InsertarjButton == null) {
			...
			InsertarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						ServicioInventarioPlaca.insertarInventarioPlacas(sqlDate);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InventarioPlacas> list = new LinkedList<InventarioPlacas>();
						try {
							list = ServicioInventarioPlaca.getInventarioPlacas();
						...
						JOptionPane.showMessageDialog(ListaInventarioPlacasVisual.this, "Fecha Insertada", "Información", JOptionPane.INFORMATION_MESSAGE);	
	...
	private JButton getModificarjButton() {
		if (ModificarjButton == null) {
			...
			ModificarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {					
					int result = JOptionPane.showConfirmDialog(ListaInventarioPlacasVisual.this, "¿Está seguro que desea modificar la Fecha?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					...
					try {
						...
						LinkedList<InventarioPlacas> users = ServicioInventarioPlaca.getInventarioPlacas();
						InventarioPlacas u = users.get(pos);	
						ServicioInventarioPlaca.ModificarInventarioPlacas(u.getId_inventarioplacas(), sqlDate);
						JOptionPane.showMessageDialog(ListaInventarioPlacasVisual.this, "Fecha Modificada", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InventarioPlacas> list = new LinkedList<InventarioPlacas>();
						try {
						...
						try {
							java.sql.Date sqlDate = new java.sql.Date(getDatespinnerDateModel().getDate().getTime());
							LinkedList<InventarioPlacas> users = ServicioInventarioPlaca.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
							InventarioPlacas u = users.get(pos);	
							ServicioInventarioPlaca.ModificarInventarioPlacas(u.getId_inventarioplacas(), sqlDate);
							JOptionPane.showMessageDialog(ListaInventarioPlacasVisual.this, "Fecha Modificada", "Información", JOptionPane.INFORMATION_MESSAGE);
							InsertarjButton.setEnabled(true);
							TableModel defaultTableModel = new TableModel();
							LinkedList<InventarioPlacas> list = new LinkedList<InventarioPlacas>();
							list = ServicioInventarioPlaca.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
							ArrayList<Object> columnDataProducto = new ArrayList<Object>();
							for (int i = 0; i < list.size(); i++) {
								columnDataProducto.add(list.get(i).getFechainventarioplacas());
								}
	...
	private JButton getEliminarjButton() {
		if (EliminarjButton == null) {
			...
			EliminarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int result = JOptionPane.showConfirmDialog(ListaInventarioPlacasVisual.this, "¿Está seguro que desea Eliminar el Inventario de Placas de Filtro?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					if(result == JOptionPane.YES_OPTION){
					int pos = jTable.getSelectedRow();
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
					try {
						LinkedList<InventarioPlacas> users = ServicioInventarioPlaca.getInventarioPlacas();
						InventarioPlacas u = users.get(pos);
						ServicioPlaca.EliminarPlacasCascada(u.getId_inventarioplacas());
						ServicioInventarioPlaca.EliminarInventarioPlacas(u.getId_inventarioplacas());						
						JOptionPane.showMessageDialog(ListaInventarioPlacasVisual.this, "Inventario  de Placas de Filtro Eliminado", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InventarioPlacas> list = new LinkedList<InventarioPlacas>();
						try {
	...
	private JSpinner getDatejSpinner() {
		if (DatejSpinner == null) {
			...
			DatejSpinner.setBounds(new Rectangle(238, 4, 136, 26));
		}
		return DatejSpinner;
	}
		...
		if (DatosjButton == null) {
			...
			DatosjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {					
					int pos = jTable.getSelectedRow();
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
					LinkedList<InventarioPlacas> users;
					try {
						users = ServicioInventarioPlaca.getInventarioPlacas();
						InventarioPlacas u = users.get(pos);
						dispose();
						new PlacasVisual(u).setVisible(true);						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					} catch (ClassNotFoundException e1) {
						...
						LinkedList<InventarioPlacas> users;
						users = ServicioInventarioPlaca.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
						InventarioPlacas u = users.get(pos);
						dispose();
						new PlacasVisual(u).setVisible(true);
					}
	...
	private JTextField getTextField() {
		if (textField == null) {
			...
			textField.setBounds(35, 43, 72, 25);
	...
	private JTextField getTextField_1() {
		if (textField_1 == null) {
			...
			textField_1.setBounds(35, 81, 72, 25);
			...
			button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					getJTable().setRowHeight(20);
							...
							JOptionPane.showMessageDialog(ListaInventarioPlacasVisual.this, "El mes debe ser entre 1 y 12", "Información", JOptionPane.ERROR_MESSAGE);
						}
					}else{
						JOptionPane.showMessageDialog(ListaInventarioPlacasVisual.this, "Los Campos Mes y Año no deben estar vacíos", "Información", JOptionPane.ERROR_MESSAGE);
					}
					
				}
			});
			button.setToolTipText("Filtrar");
			button.setIcon(new ImageIcon(ListaInventarioPlacasVisual.class.getResource("/img/system-search.png")));
			button.setBounds(0, 137, 145, 37);
		}
		return button;
	}
