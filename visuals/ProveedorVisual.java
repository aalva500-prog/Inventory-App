...
public class ProveedorVisual extends JFrame {
	...
	private void initialize() {
			...
			for (int i = 0; i < list.size(); i++) {
				columnDataProducto.add(list.get(i).getProveedor());
				}
			defaultTableModel.setRowCount(list.size());							
			defaultTableModel.addColumn("Proveedores",columnDataProducto.toArray());
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					...
					try {
						LinkedList<Proveedor> users = ServicioProveedor.getProveedores();
						Proveedor u = users.get(pos);
						jTextField.setText(u.getProveedor());
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			jLabel = new JLabel();
			jLabel.setText("Nombre Proveedor:");
			jLabel.setBounds(new Rectangle(6, 10, 121, 26));
			jPanel = new JPanel();
		...
		if (InsertarjButton == null) {
			...
			InsertarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
							ServicioProveedor.insertarProveedor(jTextField.getText());
							TableModel defaultTableModel = new TableModel();
							LinkedList<Proveedor> list = new LinkedList<Proveedor>();
							try {
								list = ServicioProveedor.getProveedores();
							...
							getJTable().setDefaultRenderer (Object.class, new FormatoTabla());		
							getJTable().setRowHeight(20);
							JOptionPane.showMessageDialog(ProveedorVisual.this, "Proveedor Insertado", "Información", JOptionPane.INFORMATION_MESSAGE);	
							UserInterfaceSuport.clearComponents(jPanel1);
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							JOptionPane.showMessageDialog(ProveedorVisual.this, "Ya Existe el Proveedor", "Información", JOptionPane.ERROR_MESSAGE);	
							e1.printStackTrace();
						} catch (ClassNotFoundException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
		...
		if (ModificarjButton == null) {
			...
			ModificarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					int result = JOptionPane.showConfirmDialog(ProveedorVisual.this, "¿Está seguro que desea modificar el Proveedor?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					if(result == JOptionPane.YES_OPTION){
					int pos = jTable.getSelectedRow();
					try {
						LinkedList<Proveedor> users = ServicioProveedor.getProveedores();
						Proveedor u = users.get(pos);	
						ServicioProveedor.Modificarproveedor(u.getId_proveedor(), jTextField.getText());
						JOptionPane.showMessageDialog(ProveedorVisual.this, "Proveedor Modificado", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<Proveedor> list = new LinkedList<Proveedor>();
						try {
	...
	private JButton getEliminarjButton() {
		if (EliminarjButton == null) {
			...
			EliminarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int result = JOptionPane.showConfirmDialog(ProveedorVisual.this, "¿Está seguro que desea Eliminar el Proveedor?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					if(result == JOptionPane.YES_OPTION){
					int pos = jTable.getSelectedRow();
					try {
						LinkedList<Proveedor> users = ServicioProveedor.getProveedores();
						Proveedor u = users.get(pos);
						ServicioProveedor.EliminarProveedor(u.getId_proveedor());
						JOptionPane.showMessageDialog(ProveedorVisual.this, "Proveedor Eliminado", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<Proveedor> list = new LinkedList<Proveedor>();
						try {
						...
						// TODO Auto-generated catch block
						JOptionPane.showMessageDialog(ProveedorVisual.this, "No se Puede Eliminar el Proveedor porque existen relaciones que lo implican", "Información", JOptionPane.ERROR_MESSAGE);
						e1.printStackTrace();
					} catch (ClassNotFoundException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
