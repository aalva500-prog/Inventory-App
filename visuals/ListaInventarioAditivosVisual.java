...
import services.ServicioInventarioAditivos;
...
public class ListaInventarioAditivosVisual extends JFrame {
	...
	public ListaInventarioAditivosVisual() {
		super();
		initialize();
	...
	private void initialize() {
			...
			this.setTitle("Listado de Inventarios de Aditivos");
			...
			LinkedList<InventarioAditivos> list = new LinkedList<InventarioAditivos>();
			try {
				list = ServicioInventarioAditivos.getInventarioAditivos();
			...
			for (int i = 0; i < list.size(); i++) {
				columnDataProducto.add(list.get(i).getFechainventarioaditivo());
				}
			defaultTableModel.setRowCount(list.size());							
			defaultTableModel.addColumn("Fechas de los Inventarios de Aditivos",columnDataProducto.toArray());
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					...
					try {
						LinkedList<InventarioAditivos> users = ServicioInventarioAditivos.getInventarioAditivos();
						InventarioAditivos u = users.get(pos);
						DatejSpinner.setValue(u.getFechainventarioaditivo());						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					} catch (ClassNotFoundException e1) {
						...
						LinkedList<InventarioAditivos> users = ServicioInventarioAditivos.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
						InventarioAditivos u = users.get(pos);
						DatejSpinner.setValue(u.getFechainventarioaditivo());
					}
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			...
			textField.setBounds(37, 43, 96, 25);
			...
			textField_1.setBounds(37, 81, 96, 25);
			textField_1.setVisible(false);
			Validate.validateDigit(textField_1);
			panel.add(textField_1);
			textField_1.setColumns(10);
			
			final JButton btnFiltrar = new JButton("");
			...
			btnFiltrar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					TableModel defaultTableModel = new TableModel();
					LinkedList<InventarioAditivos> list = new LinkedList<InventarioAditivos>();
					list = ServicioInventarioAditivos.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
					ArrayList<Object> columnDataProducto = new ArrayList<Object>();
					for (int i = 0; i < list.size(); i++) {
						columnDataProducto.add(list.get(i).getFechainventarioaditivo());
						}
					...
					getJTable().setRowHeight(20);
							...
							JOptionPane.showMessageDialog(ListaInventarioAditivosVisual.this, "El mes debe ser entre 1 y 12", "Información", JOptionPane.ERROR_MESSAGE);
						}
					}else{
						JOptionPane.showMessageDialog(ListaInventarioAditivosVisual.this, "Los Campos Mes y Año no deben estar vacíos", "Información", JOptionPane.ERROR_MESSAGE);
					}
				}
			});
			btnFiltrar.setIcon(new ImageIcon(ListaInventarioAditivosVisual.class.getResource("/img/system-search.png")));
			btnFiltrar.setBounds(0, 148, 145, 35);
		...
		if (InsertarjButton == null) {
			...
			InsertarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						ServicioInventarioAditivos.insertarInventarioAditivos(sqlDate);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InventarioAditivos> list = new LinkedList<InventarioAditivos>();
						try {
							list = ServicioInventarioAditivos.getInventarioAditivos();
						...
						JOptionPane.showMessageDialog(ListaInventarioAditivosVisual.this, "Fecha Insertada", "Información", JOptionPane.INFORMATION_MESSAGE);	
	...
	private JButton getModificarjButton() {
		if (ModificarjButton == null) {
			...
			ModificarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {					
					int result = JOptionPane.showConfirmDialog(ListaInventarioAditivosVisual.this, "¿Está seguro que desea modificar la Fecha?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					...
					try {
						...
						LinkedList<InventarioAditivos> users = ServicioInventarioAditivos.getInventarioAditivos();
						InventarioAditivos u = users.get(pos);	
						ServicioInventarioAditivos.ModificarInventarioAditivos(u.getId_inventarioaditivo(), sqlDate);
						JOptionPane.showMessageDialog(ListaInventarioAditivosVisual.this, "Fecha Modificada", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InventarioAditivos> list = new LinkedList<InventarioAditivos>();
						try {
	...
	private JButton getEliminarjButton() {
		if (EliminarjButton == null) {
			...
			EliminarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int result = JOptionPane.showConfirmDialog(ListaInventarioAditivosVisual.this, "¿Está seguro que desea Eliminar el Inventario de Aditivos?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					if(result == JOptionPane.YES_OPTION){
					int pos = jTable.getSelectedRow();
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
					try {
						LinkedList<InventarioAditivos> users = ServicioInventarioAditivos.getInventarioAditivos();
						InventarioAditivos u = users.get(pos);
						ServicioAditivo.EliminarAditivosCascada(u.getId_inventarioaditivo());
						ServicioInventarioAditivos.EliminarInventarioAditivos(u.getId_inventarioaditivo());						
						JOptionPane.showMessageDialog(ListaInventarioAditivosVisual.this, "Inventario  de Aditivos Eliminado", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InventarioAditivos> list = new LinkedList<InventarioAditivos>();
						try {
	...
	private JSpinner getDatejSpinner() {
		if (DatejSpinner == null) {
			...
			DatejSpinner.setBounds(new Rectangle(193, 4, 136, 26));
		}
		return DatejSpinner;
	}
		...
		if (DatosjButton == null) {
			...
			DatosjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {					
					int pos = jTable.getSelectedRow();
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
					LinkedList<InventarioAditivos> users;
					try {
						users = ServicioInventarioAditivos.getInventarioAditivos();
						InventarioAditivos u = users.get(pos);
						dispose();
						new AditivosVisual(u).setVisible(true);						
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					} catch (ClassNotFoundException e1) {
						...
						LinkedList<InventarioAditivos> users;
						users = ServicioInventarioAditivos.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
						InventarioAditivos u = users.get(pos);
						dispose();
						new AditivosVisual(u).setVisible(true);
					}
