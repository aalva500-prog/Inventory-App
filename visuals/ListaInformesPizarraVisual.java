...
import services.ServicioInformePizarra;
...
public class ListaInformesPizarraVisual extends JFrame {
	...
	protected JLabel lblMes=null;
	protected JLabel lblAo=null;
	protected JCheckBox chckbxNewCheckBox=null;
	public ListaInformesPizarraVisual() {
		super();
		initialize();
	...
	private void initialize() {
			...
			this.setTitle("Listado de Informes de Pizarra");
			...
			LinkedList<InformePizarra> list = new LinkedList<InformePizarra>();
			try {
				list = ServicioInformePizarra.getInformes();
			...
			for (int i = 0; i < list.size(); i++) {
				columnDataProducto.add(list.get(i).getFecha());
				}
			defaultTableModel.setRowCount(list.size());							
			defaultTableModel.addColumn("Fechas de los Informes de Pizarra",columnDataProducto.toArray());
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					...
					if(textField.getText().length()!=0 && textField_1.getText().length()!=0){
						LinkedList<InformePizarra> users = ServicioInformePizarra.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
						InformePizarra u = users.get(pos);
						DatejSpinner.setValue(u.getFecha());
					
					}
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
						try {
							LinkedList<InformePizarra> users = ServicioInformePizarra.getInformes();
							InformePizarra u = users.get(pos);
							DatejSpinner.setValue(u.getFecha());						
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
	...
	private JPanel getJPanel() {
		if (jPanel == null) {
			jLabel = new JLabel();
			jLabel.setText("Fecha del Informe de Pizarra:");
			jLabel.setBounds(new Rectangle(5, 4, 179, 26));
			jPanel = new JPanel();
			GridBagLayout gbl_jPanel = new GridBagLayout();
			gbl_jPanel.columnWidths = new int[]{494, 145, 0};
			gbl_jPanel.rowHeights = new int[]{45, 495, 0};
		...
		if (InsertarjButton == null) {
			...
			InsertarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						ServicioInformePizarra.insertarInforme(sqlDate);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InformePizarra> list = new LinkedList<InformePizarra>();
						try {
							list = ServicioInformePizarra.getInformes();
						...
						JOptionPane.showMessageDialog(ListaInformesPizarraVisual.this, "Fecha Insertada", "Información", JOptionPane.INFORMATION_MESSAGE);	
	...
	private JButton getModificarjButton() {
		if (ModificarjButton == null) {
			...
			ModificarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {					
					int result = JOptionPane.showConfirmDialog(ListaInformesPizarraVisual.this, "¿Está seguro que desea modificar la Fecha?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					...
					try {
						...
						LinkedList<InformePizarra> users = ServicioInformePizarra.getInformes();
						InformePizarra u = users.get(pos);	
						ServicioInformePizarra.ModificarInforme(u.getId_informe(), sqlDate);
						JOptionPane.showMessageDialog(ListaInformesPizarraVisual.this, "Fecha Modificada", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InformePizarra> list = new LinkedList<InformePizarra>();
						try {
						...
						try {
							...
							TableModel defaultTableModel = new TableModel();
							LinkedList<InformePizarra> list = new LinkedList<InformePizarra>();
							list = ServicioInformePizarra.getFiltro(Float.valueOf(textField.getText()),Float.valueOf(textField_1.getText()));
							ArrayList<Object> columnDataProducto = new ArrayList<Object>();
							for (int i = 0; i < list.size(); i++) {
								columnDataProducto.add(list.get(i).getFecha());
								}
	...
	private JButton getEliminarjButton() {
		if (EliminarjButton == null) {
			...
			EliminarjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int result = JOptionPane.showConfirmDialog(ListaInformesPizarraVisual.this, "¿Está seguro que desea Eliminar el Informe de Pizarra?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
					if(result == JOptionPane.YES_OPTION){
					int pos = jTable.getSelectedRow();
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
					try {
						LinkedList<InformePizarra> users = ServicioInformePizarra.getInformes();
						InformePizarra u = users.get(pos);
						ServicioDetalleInformePizarra.EliminarDetalleInformeCascada(u.getId_informe());
						ServicioInformePizarra.EliminarInforme(u.getId_informe());
						JOptionPane.showMessageDialog(ListaInformesPizarraVisual.this, "Informe de Pizarra Eliminado", "Información", JOptionPane.INFORMATION_MESSAGE);
						InsertarjButton.setEnabled(true);
						TableModel defaultTableModel = new TableModel();
						LinkedList<InformePizarra> list = new LinkedList<InformePizarra>();
						try {
	...
	private JPanel getJPanel1() {
		if (jPanel1 == null) {
			...
			panel.setBounds(3, 294, 142, 269);
			jPanel1.add(panel);
			panel.setLayout(null);
			
			final JLabel lblMes = new JLabel("Mes:");
			lblMes.setBounds(0, 43, 39, 32);
			lblMes.setVisible(false);
			panel.add(lblMes);
			
			textField = new JTextField();
			textField.setBounds(37, 43, 93, 32);
			...
			lblAo.setBounds(0, 99, 39, 32);
			panel.add(lblAo);
			
			textField_1 = new JTextField();
			textField_1.setVisible(false);
			Validate.validateDigit(textField_1);
			textField_1.setBounds(37, 99, 93, 32);
			...
			button.addActionListener(new ActionListener() {
				...
				getJTable().setRowHeight(20);
						...
						JOptionPane.showMessageDialog(ListaInformesPizarraVisual.this, "El mes debe ser entre 1 y 12", "Información", JOptionPane.ERROR_MESSAGE);
					}
				}else{
					JOptionPane.showMessageDialog(ListaInformesPizarraVisual.this, "Los Campos Mes y Año no deben estar vacíos", "Información", JOptionPane.ERROR_MESSAGE);
				}
				}
			});
			button.setToolTipText("Filtrar");
			button.setIcon(new ImageIcon(ListaInformesPizarraVisual.class.getResource("/img/system-search.png")));
			button.setBounds(0, 157, 140, 38);
			panel.add(button);
			
			final JCheckBox chckbxNewCheckBox = new JCheckBox("Filtrar");
			...
			chckbxNewCheckBox.setBounds(0, 9, 113, 25);
	...
	private JSpinner getDatejSpinner() {
		if (DatejSpinner == null) {
			...
			DatejSpinner.setBounds(new Rectangle(181, 4, 136, 26));
		}
		return DatejSpinner;
	}
		...
		if (DatosjButton == null) {
			...
			DatosjButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {		
					int pos = jTable.getSelectedRow();	
					if(textField.getText().length()!=0 && textField_1.getText().length()!=0){								
					LinkedList<InformePizarra> users;
					users = ServicioInformePizarra.getFiltro(Float.valueOf(textField.getText()), Float.valueOf(textField_1.getText()));
					InformePizarra u = users.get(pos);
					dispose();
					new DetalleInformePizarraVisual(u).setVisible(true);
					}
					if(textField.getText().length()==0 && textField_1.getText().length()==0){
						LinkedList<InformePizarra> users;
						try {
							users = ServicioInformePizarra.getInformes();
							InformePizarra u = users.get(pos);
							dispose();
							new DetalleInformePizarraVisual(u).setVisible(true);						
						} catch (SQLException e1) {
